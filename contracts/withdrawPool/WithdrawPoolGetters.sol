...
contract WithdrawPoolGetters is Proxied, Guard {
     ...
     */
    function getNumberOfTotalStakers() external view returns (uint256) {
        return
            genericDB.getUintStorage(
                CONTRACT_NAME_WITHDRAW_POOL_YIELDS,
                keccak256(abi.encodePacked("noOfTotalStakers"))
            );
    }
    ...
    function getUnlocked(uint256 _poolID) public view returns (bool) {
        return

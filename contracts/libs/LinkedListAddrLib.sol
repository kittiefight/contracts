...
/**
 * @title LinkedListAddrLib
 * @author Adopted from LinkedListLib by Darryl Morris (o0ragman0o) and Modular.network
 * 
...
library LinkedListAddrLib {
  ...
  function listExists(LinkedList storage self)
    ...
    view returns (bool)
  {
    // if the head node's previous or next pointers
    // both point to itself, then there are no items in the list
  ...
  function nodeExists(LinkedList storage self, address _node) 
    ...
    view returns (bool)
  {
    // There are two conditions where the node's prev & next pointers are zero.
    // The first one is that the node in question is the very first item in the list
    // and the second one is that there is no such a node in the list indeed.
  ...
  function sizeOf(LinkedList storage self) 
    internal
    view returns (uint256 numElements) 
  {
    bool exists;
    address i;
  ...
  function getNode(LinkedList storage self, address _node)
    internal
    view returns (bool, address, address)
  {
  ...
  {
    if (sizeOf(self) == 0) {return address(0);}
    require((_node == address(0)) || nodeExists(self,_node));
    bool exists;
    address next;
    (exists, next) = getAdjacent(self, _node, _direction);
    while ((next != address(0)) && (_value != next) && ((_value < next) != _direction)) {
  ...
  function remove(LinkedList storage self, address _node) internal returns (address) {
    if ((_node == NULL) || (!nodeExists(self,_node))) {return address(0);}
  ...
  function push(LinkedList storage self, address _node, bool _direction) internal returns (bool) {

...
contract GameCreation is Proxied, Guard {
    ...
    modifier onlyKittyOwner(address player, uint256 kittieId) {
        require(
            cryptoKitties.ownerOf(kittieId) == player,
            "Not the owner of this kittie"
        );
        _;
    }
    ...
    function initialize() external onlyOwner {
        ...
        cryptoKitties = IKittyCore(
            proxy.getContract(CONTRACT_NAME_CRYPTOKITTIES)
        );
        ...
        withdrawPool = WithdrawPool(
            proxy.getContract(CONTRACT_NAME_WITHDRAW_POOL)
        );
    ...
    function generateFight(
        ...
        (uint256 initialKTY, uint256 initialEth) = HoneypotAllocationAlgo(
            proxy.getContract(CONTRACT_NAME_HONEYPOT_ALLOCATION_ALGO)
        )
            .generateHoneyPot(gameId);
        gmSetterDB.setHoneypotInfo(gameId, initialKTY, initialEth);

        uint256 poolId = TimeFrame(proxy.getContract(CONTRACT_NAME_TIMEFRAME))
            .getActiveEpochID();

        EndowmentDB(proxy.getContract(CONTRACT_NAME_ENDOWMENT_DB))
            .setPoolIDinGame(gameId, poolId);
    ...
    {
        if (state == 0) {
            (, uint256 preStartTime, ) = gmGetterDB.getGameTimes(gameId);
            uint256 scheduledJob = cronJob.addCronJob(
                CONTRACT_NAME_GAMECREATION,
                preStartTime,
                abi.encodeWithSignature("updateGameStateCron(uint256)", gameId)
            );
        ...
        if (state == 1) {
            ...
            (uint256 startTime, , ) = gmGetterDB.getGameTimes(gameId);
            uint256 scheduledJob = cronJob.addCronJob(
                CONTRACT_NAME_GAMECREATION,
                startTime,
                abi.encodeWithSignature("callForfeiterCron(uint256)", gameId)
            );
        ...
        if (state == 2) {
            ...
            (, , uint256 endTime) = gmGetterDB.getGameTimes(gameId);
            uint256 scheduledJob = cronJob.addCronJob(
                CONTRACT_NAME_GAMECREATION,
                endTime,
                abi.encodeWithSignature("callGameEndCron(uint256)", gameId)
            );
    ...
    {
        uint256 state = gmGetterDB.getGameState(gameId);
        //Check forfeiter before updating game state
        Forfeiter(proxy.getContract(CONTRACT_NAME_FORFEITER))
            .checkGameStatusCron(gameId, state);
        state = gmGetterDB.getGameState(gameId);
        if (state == 0) gmSetterDB.updateGameStateCron(gameId);
    }

    function callForfeiterCron(uint256 gameId)
        external
        onlyContract(CONTRACT_NAME_CRONJOB)
    {
        Forfeiter(proxy.getContract(CONTRACT_NAME_FORFEITER)).forfeitCron(
            gameId,
            "Did not hit start"
        );
    }

    function callGameEndCron(uint256 gameId)
        external
        onlyContract(CONTRACT_NAME_CRONJOB)
    {
        uint256 state = gmGetterDB.getGameState(gameId);
        if (state == 2)
            GameManager(proxy.getContract(CONTRACT_NAME_GAMEMANAGER))
                .gameEndCron(gameId);
    }

    function rescheduleCronJob(uint256 gameId)
        external
        onlyContract(CONTRACT_NAME_GAMEMANAGER)
    {
        ...
        (, , uint256 endTime) = gmGetterDB.getGameTimes(gameId);
        uint256 newJobId = cronJob.rescheduleCronJob(
            CONTRACT_NAME_GAMECREATION,
            jobId,
            endTime
        );
    ...
    function deleteCronjob(uint256 gameId)
        external
        only2Contracts(
            CONTRACT_NAME_GAMEMANAGER,

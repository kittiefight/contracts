...
contract EarningsTrackerDB is Proxied, Guard {
     ...
     */
    function stopDeposits() public onlyAdmin {
     ...
     * can only be called by admin
     */
    function enableDeposits() public onlyAdmin {
     ...
     * can only be called by admin
     */
    function setCurrentFundingLimit() public onlyAdmin {
        if (getcurrentFundingLimit() == 0) {
            genericDB.setUintStorage(
                CONTRACT_NAME_EARNINGS_TRACKER_DB,
                keccak256(abi.encodePacked("currentFundingLimit")),
                fundingLimit[0]
            );
        } else {
            ...
            require(
                genericDB.getBoolStorage(
                    CONTRACT_NAME_EARNINGS_TRACKER_DB,
                    keccak256(
                        abi.encodePacked(currentGeneration, "limitReached")
                    )
                ),
            ...
            require(
                _currentFundingLimit > _prevFundingLimit,
                "Funding limit must be bigger than the previous generation"
            );
            genericDB.setUintStorage(
                CONTRACT_NAME_EARNINGS_TRACKER_DB,
                keccak256(abi.encodePacked("currentFundingLimit")),
                _currentFundingLimit
            );
        }
    }
        ...
        onlyAdmin
    {
        if (_generation == 0) {
            ...
            require(
                _fundingLimit > fundingLimit[_prevGeneration],
        ...
        onlyContract(CONTRACT_NAME_WITHDRAW_POOL)
    {
        ...
        genericDB.setUintStorage(
            CONTRACT_NAME_EARNINGS_TRACKER_DB,
            keccak256(abi.encodePacked(epoch_id, "investment")),
            _investment
        );
    }
    ...
    function setInterest(uint256 epoch_id, uint256 _total)
        ...
        onlyContract(CONTRACT_NAME_WITHDRAW_POOL)
    {
        uint256 investment = genericDB.getUintStorage(
            CONTRACT_NAME_EARNINGS_TRACKER_DB,
            keccak256(abi.encodePacked(epoch_id, "investment"))
        );
        genericDB.setUintStorage(
            CONTRACT_NAME_EARNINGS_TRACKER_DB,
            keccak256(abi.encodePacked(epoch_id, "interest")),
     ...
     */
    function getFundingLimit(uint256 _generation)
        public
        view
        returns (uint256)
    {
    ...
    function hasReachedLimit(uint256 _generation) public view returns (bool) {
        return
            genericDB.getBoolStorage(
                CONTRACT_NAME_EARNINGS_TRACKER_DB,
                keccak256(abi.encodePacked(_generation, "limitReached"))
            );
    }
        ...
        returns (uint256)
    {
        ...
        if (_startingEpoch > activeEpochID) {
            ...
            for (uint256 i = _startingEpoch; i < activeEpochID.add(1); i++) {
                uint256 investment = genericDB.getUintStorage(
                    CONTRACT_NAME_EARNINGS_TRACKER_DB,
                    keccak256(abi.encodePacked(i, "investment"))
                );
                uint256 interest = genericDB.getUintStorage(
                    CONTRACT_NAME_EARNINGS_TRACKER_DB,
                    keccak256(abi.encodePacked(i, "interest"))
                );
                uint256 epochInterest = proportion.mul(interest).div(
                    investment
                );
     ...
     */
    function KTYforBurnEthie(uint256 ethieTokenID)
        public
        view
        returns (uint256, uint256)
    {
        uint256 percentageBurnEthie = gameVarAndFee.getPercentageforBurnEthie();
        uint256 eth_amount = genericDB.getUintStorage(
            CONTRACT_NAME_EARNINGS_TRACKER,
            keccak256(abi.encodePacked(ethieTokenID, "ethValue"))
        );
        uint256 startingEpoch = genericDB.getUintStorage(
            CONTRACT_NAME_EARNINGS_TRACKER,
            keccak256(abi.encodePacked(ethieTokenID, "startingEpochID"))
        );
        uint256 withdrawAmountETH = calculateTotal(eth_amount, startingEpoch);
        uint256 withdrawAmountKTY = gameVarAndFee.convertEthToKty(
            withdrawAmountETH
        );
        uint256 burnEthieFeeKTY = withdrawAmountKTY
            .mul(percentageBurnEthie)
            .div(1000000);
        uint256 etherForSwap = KtyUniswap(
            proxy.getContract(CONTRACT_NAME_KTY_UNISWAP)
        )
            .etherFor(burnEthieFeeKTY);
    ...
    function _viewNextYieldClaimDate(uint256 ethieTokenID)
        ...
        returns (uint256)
    {
        uint256 lockTime = genericDB.getUintStorage(
            CONTRACT_NAME_EARNINGS_TRACKER,
            keccak256(abi.encodePacked(ethieTokenID, "lockTime"))
        );
        uint256 lockedAt = genericDB.getUintStorage(
            CONTRACT_NAME_EARNINGS_TRACKER,
            keccak256(abi.encodePacked(ethieTokenID, "lockedAt"))
        );
        ...
        returns (uint256)
    {
        ...
        if (_generation == 0) {
            ...
            return
                ONE_HUNDRED_AND_FIVE_DAYS.mul(_fundingLimit).div(_eth_amount);
        } else if (_generation == 5) {
            return
                ONE_HUNDRED_AND_TWENTY_DAYS.mul(_fundingLimit).div(_eth_amount);
        } else {
            //TODO: how to factor in generation 6 the funding limit of which is extremely large
            return
                ONE_HUNDRED_AND_THIRTY_FIVE_DAYS.mul(_fundingLimit).div(
                    _eth_amount
                );
        }
    }

...
contract('GameManager', (accounts) => {
  ...
  it('Test: function Claim() - Part 1', async () => {
  ...
  it('Test: function Claim() - Part 2 (WinnerShare)', async () => {

    // it would need running the full game
    for (let i = 1; i <= 4; i++) {
...
/*
    ...
    // allow claim; set state to 'claiming' i.e. 5 , and 1 min to claim
  await endowmentFund.setHoneypotState(gameId, 5, 0);
  var date = new Date(); var now_timestamp = date.getTime(); 
  await endowmentFund.setHoneypotState(gameId, 5, now_timestamp - 60); // can claim within next 1 min
*/

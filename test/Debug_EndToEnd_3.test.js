...
const GAME_PRESTART = 90; // 2 min
const GAME_DURATION = 200; // 5 min
...
let values = [GAME_PRESTART, GAME_DURATION];
...
contract("GameManager", accounts => {
  ...
  it("superDaoToken holders stake superDaoToken, and investors invest via EthieToken NFTs", async () => {
    ...
    for (let i = 0; i < 6; i++) {
      let ethAmount = web3.utils.toWei(String(5 + i), "ether");
  ...
  it("manual matches kitties", async () => {
    let kittyRed = 324;
    let kittyBlack = 1001;
    let gameStartTimeGiven = Math.floor(Date.now() / 1000) + 180; //now + 80 secs, so for prestart 30 secs 50 secs to participate
...
*/
  ...
  it("manual matches kitties", async () => {
    let kittyRed = 325;
    let kittyBlack = 1002;
    let gameStartTimeGiven = Math.floor(Date.now() / 1000) + 150; //now + 80 secs, so for prestart 30 secs 50 secs to participate
  ...
  it("mints kitties for 2 users", async () => {
    ...
    console.log(`New Player ${accounts[1]} with Kitty ${kitties[2]}`);
    ...
    console.log(`New Player ${accounts[3]} with Kitty ${kitties[3]}`);
  });
...
*/
  //============================== Game 4 Epoch 1 ==============================
  it("Starts game 4 in Epoch 1", async () => {
    console.log(
      "\n$$$$$$$$$$$$$$$$$$$$$$$$$$$$ GAME 4 $$$$$$$$$$$$$$$$$$$$$$$$$$$$"
    );
    let users = 40;
  ...
  it("mints kitties for 2 users, and user player status are unchanged after upgrading contracts", async () => {
    ...
    let kitties = [385, 1082, 1556118, 1267915, 454556, 388, 6677, 2122];
    let cividIds = [1, 2, 3, 4, 5, 6, 7, 8];
  ...
  it("manual matches kitties", async () => {
    ...
    //Take both Kitties game to see it is the same
    let gameId7 = await getterDB.getGameOfKittie(kittyRed);
    let gameId8 = await getterDB.getGameOfKittie(kittyBlack);

    console.log(gameId7.toNumber());
    console.log(gameId8.toNumber());

    if (gameId7 === gameId8) console.log("\nGameId: ", gameId7);
  ...
  it("extends the epoch end time and pool available/dissolve time dynamically depending on gaming delay", async () => {
    ...
    console.log(
      "extended epoch 1 end time in unix time:",
  ...
  it("an eligible staker of superDao tokens can claim yield from the active pool", async () => {
    ...
    console.log("total ether paid out by all past pools:", weiToEther(etherPaidOutPool1));
    console.log("-------- Stakers who have claimed from this pool ------");
  ...
  it("an investor can burn his Ethie Token NFT and receive ethers locked and interest accumulated", async () => {
    ...
    let ethie_token_ID_3 = await ethieToken.tokenOfOwnerByIndex(
      accounts[3],
      0
    );
    ethie_token_ID_3 = ethie_token_ID_3.toNumber()

    let res12 = await earningsTracker.isWorkingDay()
    console.log("Is working day?", res12)
    let stageStart2 = await earningsTracker.viewEpochStageStartTime()
    let stageEnd2 = await earningsTracker.viewEpochStageEndTime()
    console.log(`\n******************* Current Stage: ${stageStart2.state} *******************`);
    console.log("\n******************* Stage Start Time *******************");
    console.log(
      "Date:",
        stageStart2[1].toNumber() +
        "-" +
        stageStart2[2].toNumber() +
        "-" +
        stageStart2[3].toNumber(),
      " ",
      "Time:",
        stageStart2[4].toNumber() +
        ":" +
        stageStart2[5].toNumber() +
        ":" +
        stageStart2[6].toNumber()
    );
    console.log("********************************************************\n");
    console.log("\n******************* Stage End Time *******************");
    console.log(
      "Date:",
        stageEnd2[1].toNumber() +
        "-" +
        stageEnd2[2].toNumber() +
        "-" +
        stageEnd2[3].toNumber(),
      " ",
      "Time:",
        stageEnd2[4].toNumber() +
        ":" +
        stageEnd2[5].toNumber() +
        ":" +
        stageEnd2[6].toNumber()
    );
    console.log("********************************************************\n");

    await ethieToken.approve(earningsTracker.address, ethie_token_ID_3, { from: accounts[3] })
    await earningsTracker.burnNFT(ethie_token_ID_3, { from: accounts[3] }).should.be.fulfilled;
  ...
  it("sets new epoch when finalized", async () => {
    let _wait1 = await timeFrame.timeUntilEpochEnd(1);
    _wait1 = _wait1.toNumber()
    console.log(_wait1)
    await timeout(_wait1)
    // evm.increaseTime(web3, _wait)
    await proxy.executeScheduledJobs();
    console.log("Hi, new epoch!");

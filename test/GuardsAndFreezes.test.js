...
//   });
  ...
  // ================= Freeze System during Game 2 ================= 

  it("system freezes", async () => {
    console.log("KittieFight System Freezes...");
    freezeInfo.setFrozen(true);
 })

  it("players cannot bet for game 2 during system freeze", async () => {
    let gameId = 2;
    let noOfBets = 100;
    let timeInterval = 2;
  ...
  it("game cannot be finalized during system freeze", async () => {
    let gameId = 2;
  ...
  it("no on can claim during system freeze", async () => {
    let gameId = 2;
    ...
    // either playerBlack or playerRed is the hypothetical "winner"
    await proxy.execute(
      "EndowmentFund",
      setMessage(endowmentFund, "claim", [gameId]),
      {from: playerBlack}
    ...
    // do not test on top bettor/second top bettor/other bettors since no one can bet in game 2 during freeze,
    // and consequently no record on who they are - no need to test either, since claim for sure will revert due
    // to not meeting multiple conditions required in the fucntion
  });

  it("the loser in game 1 cannot redeem his/her kitty during system freeze", async () => {
    let gameId = 1;
    let winners = await getterDB.getWinners(gameId);
  ...
  it("does not add ether to pool associated with the active epoch during system freeze", async () => {
  ...
  it("an eligible staker of superDao tokens cannot claim yield from the active pool during system freeze", async () => {
    let timeTillClaiming = await withdrawPool.timeUntilClaiming();
    console.log(
      "Time (in seconds) till claiming from the current pool:",
      timeTillClaiming.toNumber()
  ...
  it("an investor cannot burn his Ethie Token NFT during system freeze", async () => {
    let tokenID = 3;

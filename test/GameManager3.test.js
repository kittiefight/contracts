...
contract('GameManager', (accounts) => {
    ...
    it('should move gameState to PRE_GAME', async () => {
        ...
        console.log('\nprestartTime: ', formatDate(gameDetails.preStartTime))

        block = formatDate(block)

        while (block < formatDate(gameDetails.preStartTime)) {
            var today = new Date();
            var time = (((today.getHours() < 10 ? '0' : '') + today.getHours()) - 1) + ":" + ((today.getMinutes() < 10 ? '0' : '') + today.getMinutes()) + ":" + ((today.getSeconds() < 10 ? '0' : '') + today.getSeconds());
            block = time;
            console.log('\n' + block + ':::' + formatDate(gameDetails.preStartTime))
            await timeout(3);
        }
    ...
    it('game not ending if bets lower than 10x starting honeypot ends', async () => {
        ...
        let block = await dateTime.getBlockTimeStamp();
        let oldEndTime = gameDetails.endTime;

        while (block < formatDate(gameDetails.endTime)) {
            var today = new Date();
            var time = (((today.getHours() < 10 ? '0' : '') + today.getHours()) - 1) + ":" + ((today.getMinutes() < 10 ? '0' : '') + today.getMinutes()) + ":" + ((today.getSeconds() < 10 ? '0' : '') + today.getSeconds());
            block = time;
            console.log('BlockTime:' + block + '+++ EndTime:' + formatDate(gameDetails.endTime))
            await timeout(6);
        }

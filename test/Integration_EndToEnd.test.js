...
contract("GameManager", accounts => {
  ...
  it("sets Epoch 0", async () => {
    ...
    const startTime = Math.floor(Date.now() / 1000) - 6 * 24 * 60 * 60 + 6 * 60;
  ...
  it("sets Pool 0", async () => {
    ...
    console.log(
      ...
      pool_0_details.ETHRemaining.toString()
    );
    console.log(
  ...
  it("investors invest via EthieToken NFTs", async () => {
  ...
  it("an eligible staker of superDao tokens can claim yield from the active pool", async () => {
    ...
    for (let i = 1; i < 4; i++) {
      await withdrawPool.claimYield({from: accounts[i]});
    }
    const pool_0_details = await withdrawPool.weeklyPools(0);
    ...
    console.log(
      "ether remaining in this pool:",
      weiToEther(pool_0_details.ETHRemaining)
    );
    console.log(
      "date available for claiming from this pool:",
      pool_0_details.dateAvailable.toString()
  ...
  it("calculates the yields for an eligible staker who can clamis from a pool", async () => {
    ...
    for (let i = 1; i < 4; i++) {
      ...
      let yields = await withdrawPool.checkYield(accounts[i]);
  ...
  it("creates a new pool", async () => {
    ...
    console.log(
      "ether remaining in this pool:",
      pool_1_details.ETHRemaining.toString()
    );
    console.log(
      "date available for claiming from this pool:",

...
contract('Betting', accounts => {
    ...
    it('is able to reduce the defense level of the opponent if each of the last 5 bets from the attacker was consecutively bigger than the previous one', async () => {
      ...
      await BettingInst.setDefenseLevel(123, accounts[1], 4)
      ...
      assert.equal(defenseLevelOppoent, 3)
    })
    ...
    it('is able to generate a fight map for a game via the function startGame()', async () => {
      ...
      const res00 = await BettingInst.fightMap.call(123, 0)
      const res01 = await BettingInst.fightMap.call(123, 1)
      const res02 = await BettingInst.fightMap.call(123, 2)
      const res03 = await BettingInst.fightMap.call(123, 3)
      const res04 = await BettingInst.fightMap.call(123, 4)
      const res05 = await BettingInst.fightMap.call(123, 5)
      const res06 = await BettingInst.fightMap.call(123, 6)
    ...
    it('calculates attack type, attackHash, current defense level of the given corner, as well as the current opponent defense level due to each bet placed by the given corner in a game', async () => {
      await BettingInst.fillBets(123, accounts[0], 20)
      await BettingInst.fillBets(123, accounts[0], 21)
      await BettingInst.fillBets(123, accounts[0], 22)
      await BettingInst.fillBets(123, accounts[0], 23)

      const numberOfBetsBN = await BettingInst.getNumberOfBets.call(123, accounts[0])
      const numberOfBets = numberOfBetsBN.toNumber()
      assert.isAtLeast(numberOfBets, 4)

      const defenseLevelOpponent1BeforeBN = await BettingInst.defenseLevel.call(
        123,
        accounts[1])
      defenseLevelOpponent1Before = defenseLevelOpponent1BeforeBN.toNumber()
      //in previous test reducelDefenseLevel(), in game 123, the opponent player accounts[1]'s defense level
      // was already reduced to 3, so here it must be eqaul to 3
      assert.equal(defenseLevelOpponent1Before, 3)

      await BettingInst.bet(
        123, // gameId
        accounts[7], // bettor
        28, // bet amount
      ...
      // since defense level of opponent before the bet being placed was 3 (> 0), and
      // the number of bets of the supported players before the bet being placed was > 3, 
      // and each bet was bigger than the previous bet in ether for 5 bets consecutively,
      // the opponent's defense level must be reduced by 1, and thus is equal to 2
      const defenseLevelOppoent1afterBN = await BettingInst.defenseLevel.call(123, accounts[1])
      defenseLevelOppoent1after = defenseLevelOppoent1afterBN.toNumber()
      assert.equal(defenseLevelOppoent1after, 2)
    })

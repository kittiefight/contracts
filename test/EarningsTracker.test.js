...
contract("EarningsTracker", accounts => {
  ...
  it("sets Epoch 0", async () => {
    ...
    await timeFrame.setEpoch_0(Math.floor(Date.now() / 1000) - 3 * 24 * 60 * 60);
  ...
  it("tells how many more ethers needed to reach the current funding limit", async () => {
    ...
    console.log("Ethers needed to reach the current funding limit", weiToEther(ethersNeeded));
  });
  ...
  it("A user can check the current stage of the current epoch, and its start and end time", async () => {
    let res1 = await earningsTracker.isWorkingDay()
  ...
  it("An investor cannot burn his Ethie Token NFT in Six Working Days in current epoch", async () => {
    let ethie_token_ID_20 = await ethieToken.tokenOfOwnerByIndex(
      accounts[20],
      0
    );
    ethie_token_ID_20 = ethie_token_ID_20.toNumber()
    await ethieToken.approve(earningsTracker.address, ethie_token_ID_20, { from: accounts[20] })
    await earningsTracker.burnNFT(ethie_token_ID_20, { from: accounts[20] }).should.be.rejected;
  })
  ...
  it("an investor can burn his Ethie Token NFT and receive ethers locked and interest accumulated", async () => {
    ...
    await earningsTracker.burnNFT(ethie_token_ID_24, { from: accounts[24] }).should.be.fulfilled;

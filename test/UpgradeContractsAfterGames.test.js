...
let medianizer = '0xA944bd4b25C9F186A846fd5668941AA3d3B8425F' //Kovan and other networks
...
contract('GameManager', (accounts) => {
    ...
    it('should upgrade contracts correctly after playing a game', async () => {
        ...
        // Upgrade GameVarAndFee
        let oldGameVarAndFee = gameVarAndFee
		console.log("old gameVarAndFee address: "+oldGameVarAndFee.address)
		gameVarAndFee = await GameVarAndFee.new(genericDB.address, medianizer)
		console.log("new gameVarAndFee address: "+gameVarAndFee.address)
		assert.notEqual(oldGameVarAndFee.address, gameVarAndFee.address)
		await proxy.removeContract('GameVarAndFee')
        await proxy.addContract('GameVarAndFee', gameVarAndFee.address)
        await gameVarAndFee.setProxy(proxy.address)

        // Upgrade GMSetterDB
        let oldSetterDB = setterDB
		console.log("old setterDB address: "+oldSetterDB.address)
		setterDB = await GMSetterDB.new(genericDB.address)
		console.log("new setterDB address: "+gameVarAndFee.address)
		assert.notEqual(oldSetterDB.address, setterDB.address)
		await proxy.removeContract('GMSetterDB')
        await proxy.addContract('GMSetterDB', setterDB.address)
        await setterDB.setProxy(proxy.address)

        // Upgrade GMGetterDB
        let oldGetterDB = getterDB
		console.log("old getterDB address: "+oldGetterDB.address)
		getterDB = await GMGetterDB.new(genericDB.address)
		console.log("new getterDB address: "+getterDB.address)
		assert.notEqual(oldGetterDB.address, getterDB.address)
		await proxy.removeContract('GMGetterDB')
        await proxy.addContract('GMGetterDB', getterDB.address)
        await getterDB.setProxy(proxy.address)

        // Upgrade GameManager
        let oldGameManager = gameManager
		console.log("old gameManager address: "+oldGameManager.address)
		gameManager = await GameManager.new()
		console.log("new gameManager address: "+gameManager.address)
		assert.notEqual(oldGameManager.address, gameManager.address)
		await proxy.removeContract('GameManager')
		...
		console.log("old scheduler address: "+oldScheduler.address)
		scheduler = await Scheduler.new()
		console.log("new scheduler address: "+scheduler.address)
		assert.notEqual(oldScheduler.address, scheduler.address)
		await proxy.removeContract('Scheduler')
        await proxy.addContract('Scheduler', scheduler.address)
        await scheduler.setProxy(proxy.address)

        // Upgrade GameCreation
        let oldGameCreation = gameCreation
		console.log("old gameCreation address: "+oldGameCreation.address)
		gameCreation = await GameCreation.new()
		console.log("new gameCreation address: "+gameCreation.address)
		assert.notEqual(oldGameCreation.address, gameCreation.address)
		await proxy.removeContract('GameCreation')
        await proxy.addContract('GameCreation', gameCreation.address)
        await gameCreation.setProxy(proxy.address)

        // Upgrade GameStore
        let oldGameStore = gameStore
		console.log("old gameStore address: "+oldGameStore.address)
		gameStore = await GameStore.new()
		console.log("new gameStore address: "+gameStore.address)
		assert.notEqual(oldGameStore.address, gameStore.address)
		await proxy.removeContract('GameStore')
        await proxy.addContract('GameStore', gameStore.address)
        await gameStore.setProxy(proxy.address)

        // Upgrade Forfeiter
        let oldForfeiter = forfeiter
		console.log("old forfeiter address: "+oldForfeiter.address)
		forfeiter = await Forfeiter.new()
		console.log("new forfeiter address: "+forfeiter.address)
		assert.notEqual(oldForfeiter.address, forfeiter.address)
		await proxy.removeContract('Forfeiter')
        await proxy.addContract('Forfeiter', forfeiter.address)
        await forfeiter.setProxy(proxy.address)

        // Upgrade KittieHellDB
        let oldKittieHellDB = kittieHellDB
		console.log("old kittieHellDB address: "+oldKittieHellDB.address)
		kittieHellDB = await KittieHellDB.new(genericDB.address)
		console.log("new kittieHellDB address: "+kittieHellDB.address)
		assert.notEqual(oldKittieHellDB.address, kittieHellDB.address)
		await proxy.removeContract('KittieHellDB')
        await proxy.addContract('KittieHellDB', kittieHellDB.address)
        await kittieHellDB.setProxy(proxy.address)

        // Upgrade KittieHELL
        let oldKittieHell = kittieHell
		...
		assert.notEqual(oldKittieHell.address, kittieHell.address)
		await proxy.removeContract('KittieHell')
    ...
    it ("upgrading contracts don't affect registered accounts", async () => {
        let users = 40;
    ...
    it("mints kitties for 2 users, and user player status are unchanged after upgrading contracts", async () => {
		let users = 8;
        ...
        assert.equal(player2CivicId.toNumber(), cividIds[1])
    ...
    it('manual matches kitties', async () => {
        ...
        console.log(gameId3.toNumber())
        console.log(gameId4.toNumber())

	    if(gameId3 === gameId4) console.log('\nGameId: ', gameId3);

	    //Take gameStartTime from blockchain to see if it is same as the one we gave
	    let {preStartTime, startTime, endTime} = await getterDB.getGameTimes(gameId3);
    ...
    it('players press start for game 2', async () => {
	    ...
	    await proxy.execute('GameManager', setMessage(gameManager, 'startGame',
	      [gameId, randomValue(22), "512955438081049600613224346938352058409509756310147795204209859701881294"]), { from: playerBlack }).should.be.fulfilled;

	    await timeout(3);

	    await proxy.execute('GameManager', setMessage(gameManager, 'startGame',
	      [gameId, randomValue(11), "24171491821178068054575826800486891805334952029503890331493652557302916"]), { from: playerRed }).should.be.fulfilled;

	    console.log('\nGame Started: ', gameId);
	    console.log('\nPlayerBlack: ', playerBlack);
        console.log('\nPlayerRed: ', playerRed);
        console.log('\nKittyBlack: ', kittyBlack.toNumber());
	    console.log('\nKittyRed: ', kittyRed.toNumber());
    })
    ...
    it('the loser can redeem his/her kitty, dynamic redemption fee is burnt to kittieHELL, replacement kitties become permanent ghosts in kittieHELL', async () => {
	    ...
	    const sacrificeKitties = [1017558, 413838, 889]

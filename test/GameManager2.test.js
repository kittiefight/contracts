...
//Kitty Ids
const kitties = [0, 1001, 155108, 1267904, 454545, 1231, 6666];
...
contract("GameManager", accounts => {
  ...
  it("approve transfer operation", async () => {
    for (let i = 1; i < 5; i++) {
      await cryptoKitties.approve(kittieHELL.address, kitties[i], {
        from: accounts[i]
      }).should.be.fulfilled;
    }
  });
  ...
  it("transfer some KTY for the test addresses", async () => {
    for (let i = 1; i < 20; i++) {
      await kittieFightToken.transfer(accounts[i], TOKENS_FOR_USERS).should.be
        .fulfilled;
    }
  });
  ...
  it("approves erc20 token transfer operation by endowment contract", async () => {
    for (let i = 1; i < 20; i++) {
      await kittieFightToken.approve(endowmentFund.address, TOKENS_FOR_USERS, {
        from: accounts[i]
      }).should.be.fulfilled;
    }
  });
  ...
  it("verify users civid Id", async () => {
    for (let i = 1; i < 5; i++) {
      await proxy.execute(
        "Register",
        setMessage(register, "verifyAccount", [cividIds[i]]),
        {
          from: accounts[i]
        }
      ).should.be.fulfilled;
  ...
  it("unverified users cannot list kitties", async () => {
    ...
    await proxy.execute(
      "GameCreation",
      setMessage(gameCreation, "listKittie", [156]),
      { from: accounts[5] }
    ).should.be.rejected;
  });
  ...
  it("is not able to list kittie without kitty ownership", async () => {
    ...
    await proxy.execute(
      "GameCreation",
      setMessage(gameCreation, "listKittie", [kitties[1]]),
      { from: accounts[3] }
    ).should.be.rejected;
  });

  it("cannot list kitties without proxy", async () => {
    await gameCreation.listKittie(kitties[1], { from: accounts[1] }).should.be
      .rejected;
  });
  ...
  it("list 3 kitties to the system", async () => {
    for (let i = 1; i < 4; i++) {
      await proxy.execute(
        "GameCreation",
        setMessage(gameCreation, "listKittie", [kitties[i]]),
        { from: accounts[i] }
      ).should.be.fulfilled;
    }
  });
  ...
  it("list 1 more kittie to the system", async () => {
    await proxy.execute(
      "GameCreation",
      setMessage(gameCreation, "listKittie", [kitties[4]]),
      { from: accounts[4] }
    ).should.be.fulfilled;
  });
  ...
  it("get correct fighter details ", async () => {
    ...
    console.log(
      ` Kittie ${kittieBlack} playing in game ${detailsBlack.gameId.toString()}`
    );

    let detailsRed = await getterDB.getFighterByKittieID(kittieRed);
    console.log(
      ` Kittie ${kittieRed} playing in game ${detailsRed.gameId.toString()}`
    );
  });
  ...
  it("bettors can participate in a created game", async () => {
    ...
    //New Supporter added
    let events = await gameManager.getPastEvents("NewSupporter", {
      fromBlock: 0,
      toBlock: "latest"
    });
    ...
    for (let i = 6; i < 18; i++) {
      ...
      await proxy.execute(
        "GameManager",
        setMessage(gameManager, "participate", [gameId, players[index]]),
        { from: accounts[i] }
      ).should.be.fulfilled;
    }

    //Check NewSupporter events
    events = await gameManager.getPastEvents("NewSupporter", {
      fromBlock: 0,
      toBlock: "latest"
    });
  ...
  it("player cant start a game before reaching PRE_GAME", async () => {
    ...
    await proxy.execute(
      "GameManager",
      setMessage(gameManager, "startGame", [gameId, 99, gene1]),
      { from: playerRed }
    ).should.be.rejected;
  });
  ...
  it("should move gameState to MAIN_GAME", async () => {
    ...
    await proxy.execute(
      "GameManager",
      setMessage(gameManager, "startGame", [gameId, 2456, gene1]),
      { from: playerRed }
    ).should.be.fulfilled;
    console.log(`\n==== PLAYER RED STARTS`);
    ...
    await proxy.execute(
      "GameManager",
      setMessage(gameManager, "startGame", [gameId, 1148, gene2]),
      { from: playerBlack }
    ).should.be.fulfilled;
    console.log(`\n==== PLAYER BLACK STARTS`);
    ...
    console.log(
      "\n==== NEW GAME STATE: ",
      gameStates[gameInfo.state.toNumber()]
    );
  ...
  it("players should be able to make bets", async () => {
    ...
    for (let i = 6; i < 29; i++) {
      ...
      await proxy.execute(
        "GameManager",
        setMessage(gameManager, "bet", [gameId, randomValue()]),
        { from: bettor, value: web3.utils.toWei(String(betAmount)) }
      ).should.be.fulfilled;
  ...
  it("correctly adds all bets for each corner", async () => {
    ...
    actualTotalBet
      .toString()
      .should.be.equal(String(web3.utils.toWei(String(totalBetAmount))));
    await timeout(1);
  });
  ...
  it("correctly computes the top bettors for each corner", async () => {
    ...
    let redSortMap = new Map(
      [...redBetStore.entries()].sort((a, b) => b[1] - a[1])
    );
    let blackSortMap = new Map(
      [...blackBetStore.entries()].sort((a, b) => b[1] - a[1])
    );
    ...
    let redSecondTopBettor = await gameStore.getSecondTopBettor(
      gameId,
      playerRed
    );

    let blackTopBettor = await gameStore.getTopBettor(gameId, playerBlack);
    let blackSecondTopBettor = await gameStore.getSecondTopBettor(
      gameId,
      playerBlack
    );
  ...
  it("game ends", async () => {
    ...
    await proxy.execute(
      "GameManager",
      setMessage(gameManager, "bet", [gameId, randomValue()]),
      { from: accounts[7], value: web3.utils.toWei("1") }
    ).should.be.fulfilled;
  ...
  it("can call finalize game", async () => {
    ...
    await proxy.execute(
      "GameManager",
      setMessage(gameManager, "finalize", [gameId, randomValue()]),
      { from: user }
    ).should.be.fulfilled;
    ...
    console.log(
      "\n====FINALIZE REWARD: ",
      web3.utils.fromWei(FINALIZE_REWARDS.toString()),
      "KTY"
    );
    console.log(
      "\n==== NEW BALANCE: ",
      web3.utils.fromWei(newBalance.toString()),
      "KTY"
    );
  ...
  it("correct honeypot info", async () => {
    ...
    honeyPotInfo.ethTotal
      .toString()
      .should.be.equal(
        String(
          ETH_PER_GAME.add(
            new BigNumber(web3.utils.toWei(String(totalBetAmount + 1)))
          )
        )
      );
  ...
  it("state changes to CLAIMING", async () => {
    ...
    console.log(
      "\n==== HONEYPOT STATE: ",
      potStates[potState.state.toNumber()]
    );
  ...
  it("show distribution details", async () => {
    ...
    console.log(
      "    ETH: ",
      web3.utils.fromWei(winnerShare.winningsETH.toString())
    );
    console.log(
      "    KTY: ",
      web3.utils.fromWei(winnerShare.winningsKTY.toString())
    );
    let topShare = await endowmentFund.getWinnerShare(
      gameId,
      winners.topBettor
    );
    ...
    console.log(
      "    ETH: ",
      web3.utils.fromWei(topShare.winningsETH.toString())
    );
    console.log(
      "    KTY: ",
      web3.utils.fromWei(topShare.winningsKTY.toString())
    );
    let secondTopShare = await endowmentFund.getWinnerShare(
      gameId,
    ...
    console.log(
      "    ETH: ",
      web3.utils.fromWei(secondTopShare.winningsETH.toString())
    );
    console.log(
      "    KTY: ",
      web3.utils.fromWei(secondTopShare.winningsKTY.toString())
    );
    let endowmentShare = await endowmentFund.getEndowmentShare(gameId);
    console.log(`  ENDOWMENT SHARE: ${rates[4].toString()} %`);
    console.log(
      "    ETH: ",
      web3.utils.fromWei(endowmentShare.winningsETH.toString())
    );
    console.log(
      "    KTY: ",
      web3.utils.fromWei(endowmentShare.winningsKTY.toString())
    );
    ...
    for (let i = 0; i < supporters.length; i++) {
      ...
      console.log(
        "    ETH: ",
        web3.utils.fromWei(share.winningsETH.toString())
      );
      console.log(
        "    KTY: ",
    ...
    let opponentShare = await endowmentFund.getWinnerShare(
      gameId,
      losers[0].supporter
    );
  ...
  it("winners can claim their share", async () => {
    ...
    let potState = await endowmentDB.getHoneypotState(gameId);
    console.log(
      "HONEYPOT DISSOLUTION TIME: ",
      formatDate(potState.claimTime.toNumber())
    );
  ...
  it("pay for resurrection", async () => {
    ...
    console.log(
      "Resurrection Cost: ",
      String(web3.utils.fromWei(resurrectionCost.toString())),
      "KTY"
    );
    ...
    await proxy.execute(
      "KittieHell",
      setMessage(kittieHELL, "payForResurrection", [loserKitty, gameId]),
      { from: loser }
    ).should.be.fulfilled;
  ...
  it("game 2 should be cancelled ", async () => {
    ...
    await proxy.execute(
      "GameManager",
      setMessage(gameManager, "participate", [2, gamePlayers.playerBlack]),
      { from: accounts[18] }
